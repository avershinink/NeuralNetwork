...
namespace NeuralNetworkTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < testingData.Length; i++)
            {
                ...
                for (int j = 0; j < result.Capacity; j++)
                {
                    ...
                    if (testingDataOutputData[i][j] == 1) rightAnswerIndex = j;
                }
        ...
        private static void Shuffle(double[][] entries, int TestingDataElementsCount, out double[][] LearningData, out double[][] ExpectedOutputData, out double[][] TestingData, out double[][] TestingDataExpectedOutput)
        {
            var randomIndex    =    new int[entries.Length];
            LearningData       = new double[entries.Length - TestingDataElementsCount][];
            ExpectedOutputData = new double[entries.Length - TestingDataElementsCount][];
            TestingData               = new double[TestingDataElementsCount][];
            TestingDataExpectedOutput = new double[TestingDataElementsCount][];
            ...
            for (int i = 0; i < randomIndex.Length; i++)
            {
                int r = rnd.Next(i, randomIndex.Length);
                randomIndex[i] = randomIndex[r];
                randomIndex[r] = i;
            }

            for (int i = 0; i < randomIndex.Length - TestingDataElementsCount; i++)
            {
                LearningData[i] = new double[] { entries[randomIndex[i]][0], entries[randomIndex[i]][1], entries[randomIndex[i]][2], entries[randomIndex[i]][3] };
                ExpectedOutputData[i] = new double[] { entries[randomIndex[i]][4], entries[randomIndex[i]][5], entries[randomIndex[i]][6] };
            }
            var j = 0;
            for (int i = randomIndex.Length - TestingDataElementsCount; i < randomIndex.Length; i++)
            {
                TestingData[j] = new double[] { entries[randomIndex[i]][0], entries[randomIndex[i]][1], entries[randomIndex[i]][2], entries[randomIndex[i]][3] };
                TestingDataExpectedOutput[j++] = new double[] { entries[randomIndex[i]][4], entries[randomIndex[i]][5], entries[randomIndex[i]][6] };
            }
        }

        private static double[][] getInputs(bool resultsToMinus = false)
        {
            ...
            foreach (double[] item in allData)
            {
                if (item[4] == 0) item[4] = -1;
                if (item[5] == 0) item[5] = -1;
                if (item[6] == 0) item[6] = -1;
            }
